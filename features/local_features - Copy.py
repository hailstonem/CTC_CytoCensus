...
def local_shape_features_pyramid_cv2(im, scaleStart):
    """ Creates features based on those in the Luca Fiaschi paper but on 5 scales independent of object size,
    but using a gaussian pyramid to calculate at multiple scales more efficiently
    and then linear upsampling to original image scale
    also includes gaussian smoothed image
    ...
    for layer in range(0, 5):
        if layer>0:
            blur=cv2.GaussianBlur(im,(1+4*(2**layer),1+4*(2**layer)),(2**layer))
            a = cv2.resize(blur, tuple([int(x/(2**layer)) for x in im.shape[::-1]]))

        # create features
        lap = cv2.Laplacian(a, -1)
       #lap = scipy.ndimage.filters.laplace(a)
        #m = cv2.Sobel(a, -1, 1,0)
        #n = cv2.Sobel(a, -1, 0,1)
        [m, n] = np.gradient(a)
        ...
        # upsample features to original image
        lap = cv2.resize(lap, im.shape[::-1])
        ggm = cv2.resize(ggm, im.shape[::-1])
        st0 = cv2.resize(st[0], im.shape[::-1])
        st1 = cv2.resize(st[1], im.shape[::-1])
        up = cv2.resize(a, im.shape[::-1])
